...
#   2. Clone your current Helios repo (or copy this package to the host)
...
if [ -d "$XIPHOS_DIR/.git" ]; then
    ...
    if [ -d ".git" ]; then
        ...
        echo "  ERROR: Run this script from inside the Helios git repo,"
        echo "         or clone your current deployment repository first."
        exit 1
    fi
fi
...
ExecStart=/usr/bin/docker compose exec -T -w /app/backend xiphos python3 -c "import sanctions_sync; sanctions_sync.init_sanctions_db(); sanctions_sync.sync_all()"
SVCEOF
