...
def _render_markdown(summary: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- Masked holdout hits@10: `{missing_edge_metrics.get('masked_holdout_hits_at_10', 0.0):.2f}`",
        f"- Masked holdout MRR: `{missing_edge_metrics.get('masked_holdout_mrr', 0.0):.2f}`",
        f"- Mean withheld target rank: `{missing_edge_metrics.get('mean_withheld_target_rank', 0.0):.2f}`",
        f"- Holdout queries evaluated: `{missing_edge_metrics.get('masked_holdout_queries_evaluated', 0)}`",
