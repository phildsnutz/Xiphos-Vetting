...
def enrich(vendor_name: str, country: str = "", **ids) -> EnrichmentResult:
    ...
    try:
        ...
        if entity:
            ...
            if founded: detail_parts.append(f"Founded: {founded}")
            if hq: detail_parts.append(f"HQ: {hq}")
            if industry: detail_parts.append(f"Industry: {industry}")
            if exchange: detail_parts.append(f"Exchange: {exchange}")
            if employees: detail_parts.append(f"Employees: {employees}")
            if parent: detail_parts.append(f"Parent: {parent}")
            if entity_country: detail_parts.append(f"Country: {entity_country}")
            ...
            result.findings.append(Finding(
                ...
                url=f"https://www.wikidata.org/wiki/{wikidata_id}" if wikidata_id else "",
            ))
