...
def _search_local_db(vendor_name: str) -> list[dict]:
    ...
    try:
        ...
        if "sanctions_entities" in tables:
            cur.execute(
                """
                SELECT DISTINCT se.name, se.country, se.program, se.list_type, se.remarks
                FROM sanctions_entities se
                LEFT JOIN sanctions_aliases sa ON sa.entity_id = se.id
                WHERE (
                    se.source LIKE '%worldbank%' OR
                    se.program LIKE '%world bank%' OR
                    se.list_type LIKE '%world bank%' OR
                    se.remarks LIKE '%world bank%'
                )
                AND (
                    se.name LIKE ? OR sa.alias LIKE ?
                )
                LIMIT 25
                """,
                (search, search)
