...
def _fallback_local_search(name: str) -> list[dict]:
    ...
    try:
        ...
        cursor.execute(
            ...
            LEFT JOIN sanctions_aliases sa ON sa.entity_id = se.id
            WHERE se.source = 'opensanctions'
              AND (
                    se.name = ? OR se.name LIKE ? OR
                    sa.alias = ? OR sa.alias LIKE ?
              )
            LIMIT 20
            """,
...
def _extract_other_risks(entity: dict) -> list[str]:
    ...
    # Local fallback rows come from the sanctions dataset and lack topic metadata.
    if not risks and entity.get("source") == "opensanctions":
        if entity.get("program") or entity.get("list_type") or entity.get("remarks"):
            risks.append("sanction")
