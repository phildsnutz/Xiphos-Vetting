...
def generate_dossier(vendor_id: str, user_id: str = "", hydrate_ai: bool = False) -> str:
    ...
    hero_section = _generate_executive_summary(
        vendor,
        score,
        enrichment,
        monitoring_history=monitoring_history,
    ...
    chapter_nav = _generate_dossier_chapter_nav()
    decision_brief = _wrap_dossier_chapter(
        "Chapter 1",
        "Decision brief",
        "Start with the strongest recommendation, the shortest risk storyline, and the portable trust artifact before you drop into raw evidence.",
        "".join(
            [
                _generate_storyline_section(storyline),
                _generate_supplier_passport_section(supplier_passport),
            ]
        ),
        tone="dark",
    )
    control_evidence = _wrap_dossier_chapter(
        "Chapter 2",
        "Control evidence",
        "Identity, ownership, graph provenance, and lane-specific evidence that explains why the current posture should be believed or challenged.",
        "".join(
            [
                _generate_graph_provenance_section(graph_summary),
                _generate_foci_evidence_section(foci_summary),
                _generate_cyber_evidence_section(cyber_summary),
                _generate_export_evidence_section(export_summary),
                _generate_scoring_breakdown(score),
                _generate_alert_disposition_section(score),
                _generate_itar_compliance_section(score),
            ]
        ),
        tone="light",
    )
    analyst_narrative = _wrap_dossier_chapter(
        "Chapter 3",
        "Analyst narrative",
        "This is the human-readable interpretation layer: AI judgment, intelligence synthesis, and the evidence snapshot a reviewer is likely to carry into the decision meeting.",
        "".join(
            [
                _generate_ai_narrative(vendor_id, vendor, analysis_data=analysis_data),
                _generate_intel_summary_section(intel_summary),
                _generate_key_evidence_snapshot(enrichment),
                _generate_recommended_actions(score),
            ]
        ),
        tone="light",
    )
    appendix = _wrap_dossier_chapter(
        "Chapter 4",
        "Operations appendix",
        "Full event, finding, and freshness material for auditability. This is where the dossier stops selling and starts proving.",
        "".join(
            [
                _generate_normalized_events(case_events),
                _generate_osint_findings(enrichment),
                _generate_data_freshness(enrichment, score),
    ...
    sections = [hero_section, chapter_nav, decision_brief, control_evidence, analyst_narrative, appendix]
        ...
        <style>
            ...
            * {{
            ...
            body {{
                ...
                color: #1a1f36;
                background:
                    radial-gradient(circle at top, rgba(196, 160, 82, 0.08), transparent 32%),
            ...
            .container {{
                ...
                margin: 24px auto;
                background:
                    linear-gradient(180deg, rgba(249, 250, 251, 0.8), rgba(255, 255, 255, 0.98) 120px),
                    white;
                padding: 0.9in;
            ...
            .hero-panel {{
                background:
                    radial-gradient(circle at top right, rgba(196, 160, 82, 0.22), transparent 28%),
                    linear-gradient(135deg, var(--navy-900), var(--navy-950));
                border-radius: 22px;
                padding: 28px;
                color: white;
                box-shadow: inset 0 0 0 1px rgba(196, 160, 82, 0.16);
            }}
            ...
            .storyline-callout {{
                ...
                padding: 14px 16px;
                border-radius: 16px;
                background: linear-gradient(135deg, rgba(10, 22, 40, 0.96), rgba(16, 32, 51, 0.94));
                color: rgba(255, 255, 255, 0.82);
                font-size: 11px;
                line-height: 1.55;
            ...
            .storyline-card::before {{
                ...
                inset: 0 auto 0 0;
                width: 4px;
                background: var(--storyline-accent, var(--gold-500));
            }}
            ...
            .chapter-shell {{
                ...
                border-radius: 24px;
                border: 1px solid rgba(16, 32, 51, 0.08);
                box-shadow: 0 18px 48px rgba(10, 22, 40, 0.07);
            }}
            ...
            .chapter-shell-dark {{
                background:
                    radial-gradient(circle at top right, rgba(196, 160, 82, 0.18), transparent 24%),
                    linear-gradient(180deg, #0d1726 0%, #122033 100%);
                color: white;
            }}

            .chapter-shell-appendix {{
            ...
            .chapter-shell-dark .chapter-head {{
            ...
            .chapter-shell-dark .chapter-summary,
            .chapter-shell-dark .chapter-kicker {{
                color: rgba(255, 255, 255, 0.82);
            }}
            ...
            .chapter-shell-dark .chapter-body > section {{
    ...
    <body>
        <div class="watermark">HELIOS</div>
