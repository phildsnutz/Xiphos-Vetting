...
def _get_auth_db_path():
    base = os.environ.get("XIPHOS_DB_PATH",
...
def create_user(email: str, password: str, name: str = "",
    ...
    try:
        conn.execute("""
            INSERT INTO users (id, email, name, role, password_hash, salt)
            VALUES (?, ?, ?, ?, ?, ?)
        """, (user_id, email, name, role, pw_hash, salt))
        conn.commit()
...
def require_auth(permission: str):
    ...
    When AUTH_ENABLED is False, all requests pass through with a
    synthetic admin identity (development mode).
    """
    def decorator(f):
        @functools.wraps(f)
