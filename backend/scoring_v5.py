"""
Xiphos Risk Scoring Engine v5.0
Sensitivity-Aware FGAMLogit (Factored Generalized Additive Logistic Model)

Clean-break replacement for the Beta(2,8) Bayesian engine.
...
Design:
  ...
  - Marginal Information Values (MIV) via analytic gradient of logistic function
  - Full backward-compatible output via ScoringResultV5 dataclass

Model version: 3.0-FGAMLogit-DoD-Dual-Vertical
...
def _logit(p: float) -> float:
    """Log-odds of probability p. Clamps to avoid ±inf."""
    p = max(1e-9, min(1.0 - 1e-9, p))
    return math.log(p / (1.0 - p))


def _wilson_ci(p_hat: float, n_effective: float, z: float = 1.96) -> tuple[float, float]:
    """
...
# ─────────────────────────────────────────────────────────────────────────────
# GEOGRAPHY & FACTOR HELPERS (unchanged from v3 -- same lookup, not the scorer)
# ─────────────────────────────────────────────────────────────────────────────
...
class OwnershipProfile:
    ...
    foreign_ownership_pct: float = 0.0          # [0-1]
    foreign_ownership_is_allied: bool = True     # True = US-allied country owner
...
class DoDContext:
    ...
    regulatory_gate_proximity: float = 0.0 # 0-1: from Layer 1 gate_proximity_score
    itar_exposure: float = 0.0             # 0-1: see factor definition
    ear_control_status: float = 0.0        # 0-1: see factor definition
    foreign_ownership_depth: float = 0.0   # 0-1: computed from ownership profile
    cmmc_readiness: float = 0.0            # 0-1: distance to required CMMC level
    single_source_risk: float = 0.0        # 0-1: see factor definition
    geopolitical_sector_exposure: float = 0.0  # 0-1: sector sensitivity
    financial_stability: float = 0.2       # 0-1: financial health risk (low = healthy)
    compliance_history: float = 0.0        # 0-1: past regulatory violations
...
# ─────────────────────────────────────────────────────────────────────────────
# MODEL PARAMETERS  (all weights are manually calibrated, no ML fitting)
# ─────────────────────────────────────────────────────────────────────────────

# Sensitivity tiers in order from most to least scrutiny
SENSITIVITY_TIERS = ["SAP", "SCI", "TOP_SECRET", "SECRET", "CUI", "UNCLASSIFIED", "COMMERCIAL"]

# β₀: baseline log-odds per sensitivity tier
# Reflects inherent scrutiny level; higher → higher baseline risk probability
BASELINE_LOGODDS: dict[str, float] = {
    "SAP":          0.50,   # logistic(0.50) ≈ 0.62 — maximum scrutiny baseline
    "SCI":          0.30,   # ≈ 0.57
    "TOP_SECRET":   0.10,   # ≈ 0.52
    "SECRET":      -0.41,   # ≈ 0.40
    "CUI":         -1.39,   # ≈ 0.20
    "UNCLASSIFIED":-2.94,   # ≈ 0.05
    "COMMERCIAL":  -2.94,   # ≈ 0.05
}

# Factor weight matrix: FACTOR_WEIGHTS[factor_name][sensitivity_tier]
# Columns: SAP, SCI, TOP_SECRET, SECRET, CUI, UNCLASSIFIED, COMMERCIAL
...
# Interaction terms: INTERACTION_WEIGHTS[(factor_a, factor_b)][sensitivity]
# Applied as: γ * x_a * x_b added to log-odds
...
# Effective sample size for Wilson CI by sensitivity tier
# Smaller cohort → wider CI (more uncertainty)
...
# ─────────────────────────────────────────────────────────────────────────────
# FACTOR COMPUTATION  (raw → normalized 0-1 per factor)
# ─────────────────────────────────────────────────────────────────────────────
...
def _compute_foreign_ownership_depth(o: OwnershipProfile) -> float:
    """
    Combines foreign ownership percentage and country-of-origin risk into 0-1.
    Used as a DoD factor distinct from the commercial 'ownership' factor.
    """
...
def _evaluate_hard_stops(
    ...
    These always produce TIER_1 regardless of FGAMLogit output.
    """
    stops = []
    hard_composite_threshold = 0.82  # v3.0 recalibrated threshold

    # Sanctions hard stop
    if screening.matched and screening.best_score > hard_composite_threshold:
        stops.append({
            ...
            "explanation": (
                ...
                f"{screening.matched_entry.program} — "
    ...
    country_risk = geo_risk(country)
    if ownership.state_owned and country_risk > 0.50:
        stops.append({
            "trigger": "Adversary State-Owned Enterprise",
            "explanation": (
                f"State-owned entity in sanctioned/adversarial jurisdiction ({country}). "
                "Per EO 13959 and NDAA Section 1260H, SOEs from adversary nations are "
                "disqualified from DoD supply chains."
            ),
            "confidence": 0.92,
        })

    # SAP/SCI with any foreign ownership
    if sensitivity in ("SAP", "SCI") and ownership.foreign_ownership_pct > 0.0:
        stops.append({
            ...
            "explanation": (
                ...
                "and cannot participate without full restructuring."
            ),
            "confidence": 0.99,
        })
...
def _evaluate_soft_flags(
    ...
    flags = []
    fuzzy_lo = 0.60
    fuzzy_hi = 0.82  # v3.0 recalibrated

    if screening.matched and fuzzy_lo < screening.best_score <= fuzzy_hi:
        flags.append({
            "trigger": "Fuzzy Sanctions Match",
            "explanation": (
                f"Name similarity {screening.best_score * 100:.0f}% to "
    ...
    if exec_profile.adverse_media > 0:
        flags.append({
            ...
            "explanation": (
                f"{exec_profile.adverse_media} adverse media hit(s) detected "
                "on executive screening."
            ),
            "confidence": 0.70,
        })
    if data_quality.years_of_records < 3:
        flags.append({
            ...
            "explanation": (
                f"Entity has only {data_quality.years_of_records} year(s) of "
                "verifiable records."
            ),
            "confidence": 0.85,
        })
    if dod.regulatory_gate_proximity >= 0.5:
        flags.append({
            ...
            "explanation": (
                ...
                "one or more compliance gates are approaching failure state."
            ),
            "confidence": 0.88,
        })
    if dod.cmmc_readiness >= 0.5 and dod.sensitivity in ("SAP", "SCI", "TOP_SECRET", "SECRET", "CUI"):
        flags.append({
            ...
            "explanation": (
                f"Entity is {dod.cmmc_readiness * 100:.0f}% of the way to a CMMC "
                "certification failure. Remediation timeline may not meet contract schedule."
            ),
            "confidence": 0.82,
        })
    if dod.single_source_risk >= 0.6:
        flags.append({
            ...
            "explanation": (
                "Entity is sole or single source for a critical component. Supply chain "
                "resilience is significantly compromised if this supplier fails."
            ),
            "confidence": 0.75,
        })
    return flags
...
# ─────────────────────────────────────────────────────────────────────────────
# MARGINAL INFORMATION VALUES  (analytic gradient)
# ─────────────────────────────────────────────────────────────────────────────
...
def _compute_mivs(
    ...
    """
    Marginal Information Value for each uncertain factor.
    MIV_j = |∂p/∂x_j| = p(1-p) * |effective_weight_j|
    where effective_weight accounts for interactions.

    Returns only factors where gaining perfect information
    could shift probability by >= 1 percentage point.
    """
    p_var = probability * (1.0 - probability)
    mivs = []
    ...
    for fname, fx in factor_scores.items():
        ...
        # Add interaction contribution to effective weight
        effective_w = w
        for (fa, fb), iweights in INTERACTION_WEIGHTS.items():
            ...
            if iw == 0.0:
                continue
            if fa == fname:
                effective_w += iw * factor_scores.get(fb, 0.0)
            elif fb == fname:
                effective_w += iw * factor_scores.get(fa, 0.0)

        gradient = abs(p_var * effective_w)
        shift_pp = gradient * 100.0  # convert to percentage points

        if shift_pp >= 1.0:
            # Estimate tier-change probability: rough rule based on shift size
            tier_prob = min(0.90, shift_pp / 50.0)
            mivs.append({
                "factor": fname,
    ...
    mivs.sort(key=lambda m: m["expected_shift_pp"], reverse=True)
    return mivs
...
def _factor_description(
    ...
    if factor == "sanctions":
        if screening.matched:
            return (
                ...
                f'{screening.best_score * 100:.0f}% composite similarity'
            )
    ...
    if factor == "executive":
        if inp.exec_profile.known_execs == 0: return "No executive data available — screening incomplete."
    ...
    if factor == "regulatory_gate_proximity":
        ...
        if score < 0.5: return f"Gate proximity {score:.2f} — one or more gates in PENDING state, remediation on track."
        return f"Gate proximity {score:.2f} — multiple gates PENDING or approaching failure."
    if factor == "itar_exposure":
        if score == 0.0: return "Non-ITAR item — ITAR gate not applicable."
        return f"ITAR exposure score {score:.2f} — item is ITAR-controlled, Tier {inp.dod.supply_chain_tier} accountability."
    ...
    if factor == "cmmc_readiness":
        ...
        return f"CMMC readiness gap score {score:.2f} — certification distance from program requirement."
    if factor == "single_source_risk":
        ...
        return f"Single/sole-source critical component (score {score:.2f}) — supply continuity risk."
    if factor == "geopolitical_sector_exposure":
        ...
        return f"High geopolitical sector exposure (score {score:.2f}) — adversary-adjacent technology or location."
    if factor == "financial_stability":
        if score < 0.2: return "Strong financial position — low continuity risk."
    ...
    if factor == "compliance_history":
        if score < 0.1: return "Clean compliance record — no violations."
        if score < 0.4: return f"Minor historical violations, resolved (score {score:.2f})."
        return f"Pattern of compliance violations (score {score:.2f}) — regulatory reliability concern."
...
class ScoringResultV5:
    ...
    Fields marked [Layer 1] require regulatory_gates.py to be called first
    and passed in via the regulatory_ prefix fields.
    """
    # ── Core probabilistic output ─────────────────────────────────────────
    calibrated_probability: float          # FGAMLogit output, 0-1
    calibrated_tier: str                   # TIER_1..4 from integrate_layers()
    interval_lower: float
    interval_upper: float

    # ── Factor analysis ───────────────────────────────────────────────────
    contributions: list[dict]              # Per-factor with signed contribution
    ...
    # ── [Layer 1] Regulatory gate results (optional) ──────────────────────
    regulatory_status: str = "NOT_EVALUATED"   # COMPLIANT / NON_COMPLIANT / REQUIRES_REVIEW
    regulatory_findings: list[dict] = field(default_factory=list)

    # ── Metadata ──────────────────────────────────────────────────────────
    model_version: str = "3.0-FGAMLogit-DoD-Dual-Vertical"
...
def integrate_layers(
    ...
    """
    Map (regulatory_status × risk_probability × sensitivity) → combined_tier.

    Tiers:
      TIER_1_DISQUALIFIED          — Non-compliant; cannot proceed
      TIER_1_CRITICAL_CONCERN      — Regulatory issues + high probability
      TIER_2_ELEVATED_REVIEW       — Requires review + moderate probability
      TIER_2_CONDITIONAL_ACCEPTABLE — Requires review + low probability
      TIER_2_ELEVATED_CONCERN      — Compliant SAP/SCI but probability elevated
      TIER_2_ELEVATED              — Compliant TS but probability elevated
      TIER_2_CAUTION               — Compliant SECRET/CUI but probability elevated
      TIER_2_CAUTION_COMMERCIAL    — Commercial with elevated probability
      TIER_3_SAP_ACCEPTABLE        — SAP compliant, moderate probability
      TIER_3_CONDITIONAL           — TS or SECRET/CUI compliant, moderate probability
      TIER_4_SAP_QUALIFIED         — SAP compliant, low probability — approved
      TIER_4_APPROVED              — Compliant, low probability — approved
      TIER_4_CLEAR                 — Commercial, very low probability
    """
    ...
    # COMPLIANT — paths vary by sensitivity
...
def _program_recommendation(
    ...
    # Hard disqualification (Section 889, CMC list, etc.)
    if regulatory_status == "NON_COMPLIANT" or combined_tier == "TIER_1_DISQUALIFIED":
        return "DO_NOT_PROCEED"
    # Regulatory gates in progress with high probabilistic risk
    ...
    if regulatory_status == "REQUIRES_REVIEW":
        ...
        return "DO_NOT_PROCEED_WITHOUT_MITIGATION"
    # COMPLIANT — approval level based on risk probability
    if risk_probability < 0.25:
        return "APPROVED"
    if risk_probability < 0.40:
...
def score_vendor(
    ...
    Args:
        ...
        regulatory_status:   Output from regulatory_gates.evaluate_regulatory_gates()
                             ("COMPLIANT" / "NON_COMPLIANT" / "REQUIRES_REVIEW")
                             Pass "NOT_EVALUATED" to skip layer integration.
    ...
    # DoD factors — use provided values from DoDContext (caller responsible for normalization)
    # If ownership profile has foreign_ownership data, also use that for foreign_ownership_depth
    ...
    for (fa, fb), iweights in INTERACTION_WEIGHTS.items():
        ...
        if iw == 0.0:
            continue
        xa = factor_scores.get(fa, 0.0)
        xb = factor_scores.get(fb, 0.0)
        eta += iw * xa * xb
    ...
    # Hard stops floor the probability and fix tier
    if stops:
        probability = max(probability, 0.82)  # v3.0 hard_stop_threshold

    # ── Step 5: Tier from layer integration ───────────────────────────────
    if regulatory_status == "NOT_EVALUATED":
        # Fallback: tier from probability alone (commercial mode)
    ...
    # ── Step 6: Confidence interval (Wilson score with effective n) ───────
    n_base = EFFECTIVE_N_BASE.get(sensitivity, 100.0)
    if regulatory_status == "REQUIRES_REVIEW":
        n_eff = n_base * 0.70
    elif regulatory_status == "NOT_EVALUATED":
        n_eff = n_base
    else:
        n_eff = n_base
    ...
    for fname, fx in factor_scores.items():
        ...
        # Counterfactual probability without this factor
    ...
    flags = _evaluate_soft_flags(
        screening, inp.ownership, inp.exec_profile, inp.data_quality, inp.dod
    )
    ...
    if stops:
        findings.append(
            ...
            "cannot be mitigated through risk controls."
        )
    if probability >= 0.60:
        findings.append(
            f"FGAMLogit probability of {probability * 100:.1f}% indicates substantial "
            "compliance risk requiring immediate escalation."
        )
    ...
    if regulatory_status == "NON_COMPLIANT":
        findings.append(
            "Entity is NON_COMPLIANT with one or more mandatory DoD regulatory gates. "
            "Disqualified from this supply chain regardless of probabilistic score."
        )
    elif regulatory_status == "REQUIRES_REVIEW":
        findings.append(
            "One or more regulatory gates are in PENDING state. Compliance is achievable "
            "but requires documented remediation before contract award."
        )
    ...
    if not inp.ownership.beneficial_owner_known:
        findings.append(
            "Beneficial ownership unresolved — enhanced due diligence required before "
            "any sensitive contract."
        )
    if inp.dod.single_source_risk >= 0.8:
        findings.append(
            "Sole-source critical component per DoD Instruction 5200.44. Supply chain "
            "resilience must be independently assessed."
        )
    if flags:
        findings.append(f"{len(flags)} advisory flag(s) warrant analyst review before contract award.")
    ...
    return ScoringResultV5(
        ...
        model_version="3.0-FGAMLogit-DoD-Dual-Vertical",
    )
...
@dataclass
class OwnershipProfileLegacy:
    """Maps old OwnershipProfile fields — no foreign_ownership fields required."""
    publicly_traded: bool = False
    state_owned: bool = False
    beneficial_owner_known: bool = False
    ownership_pct_resolved: float = 0.0
...
class VendorInput:
    ...
    name: str
    country: str
    ownership: OwnershipProfileLegacy
    data_quality: DataQuality
    exec_profile: ExecProfile
    program: str = "standard_industrial"


# Map old program strings to nearest sensitivity tier
...
def score_vendor_legacy(inp: VendorInput) -> "ScoringResultV5":
    """
    Backward-compatible wrapper so existing server.py call sites
    (score_vendor(VendorInput(...))) continue to work unchanged.
    """
